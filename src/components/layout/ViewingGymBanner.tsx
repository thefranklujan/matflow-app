...
export default function ViewingGymBanner({ gymName }: { gymName: string }) {
  const [busy, setBusy] = useState(false);

  async function exit() {
    if (!confirm(`Exit "Viewing as ${gymName}" and return to the Platform Dashboard?`)) return;
    setBusy(true);
    const res = await fetch("/api/platform/impersonate/exit", { method: "POST" });
    if (res.ok) {
      window.location.href = "/platform/gyms";
    } else {
  ...
  return (
    <div className="bg-orange-500 text-black px-6 py-2 flex items-center justify-between text-sm font-semibold shrink-0">
      <span className="inline-flex items-center gap-2">
        ...
        Viewing as <strong>{gymName}</strong>
      </span>
      <button
        onClick={exit}
        disabled={busy}
      ...
      >
        {busy ? "Exiting..." : `Exit Viewing as ${gymName}`}
      </button>
    </div>
  );
}
