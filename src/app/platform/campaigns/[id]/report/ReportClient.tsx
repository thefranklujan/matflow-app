...
export default function ReportClient() {
  ...
  if (loading) {
    return <div className="text-gray-400 text-sm py-12 text-center">Loading report...</div>;
  }

  if (!data?.campaign) {
    return <div className="text-gray-400 text-sm py-12 text-center">Campaign not found.</div>;
  }
    ...
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4" style={{ marginBottom: "32px" }}>
        ...
        <KPI label="Bounced" value={metrics.failed} sub={metrics.sent > 0 ? `${Math.round((metrics.failed / metrics.sent) * 100)}% bounce rate` : undefined}
          icon={AlertTriangle} color={metrics.failed > 0 ? "text-red-400" : "text-gray-400"} bgColor={metrics.failed > 0 ? "bg-red-500/10" : "bg-white/5"}
        ...
        <KPI label="Unsubscribed" value={metrics.unsubscribed}
          icon={UserX} color={metrics.unsubscribed > 0 ? "text-orange-400" : "text-gray-400"} bgColor={metrics.unsubscribed > 0 ? "bg-orange-500/10" : "bg-white/5"}
        ...
        <div className="bg-[#111] border border-white/10 rounded-xl overflow-hidden" style={{ marginBottom: "32px" }}>
          <div className="px-6 py-4 border-b border-white/10 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <span className="text-xs text-gray-400">{data.emailLists[activeList].length} emails</span>
              <button onClick={() => setActiveList(null)} className="text-gray-400 hover:text-white transition">
                <X className="h-4 w-4" />
              </button>
            </div>
          </div>
          <div className="max-h-[300px] overflow-y-auto divide-y divide-white/5">
            {data.emailLists[activeList].length === 0 ? (
              <p className="px-6 py-8 text-gray-400 text-sm text-center">No emails in this category yet</p>
            ) : (
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="bg-[#111] border border-white/10 rounded-xl overflow-hidden">
          <div className="px-6 py-4 border-b border-white/10 flex items-center justify-between">
            ...
            <span className="text-xs text-gray-400">{metrics.totalClicks} total clicks</span>
          </div>
          <div className="divide-y divide-white/5">
            {clicksByUrl.length === 0 ? (
              <p className="px-6 py-8 text-gray-400 text-sm text-center">No clicks yet</p>
            ) : (
        ...
        <div className="bg-[#111] border border-white/10 rounded-xl overflow-hidden">
          ...
          <div className="divide-y divide-white/5 max-h-[400px] overflow-y-auto">
            {recentActivity.length === 0 ? (
              <p className="px-6 py-8 text-gray-400 text-sm text-center">No activity yet</p>
            ) : (
...
function KPI({ label, value, sub, icon: Icon, color, bgColor, active, onClick }: {
  ...
  return (
    ...
    >
      ...
      {sub && <div className="text-xs text-gray-400" style={{ marginTop: "4px" }}>{sub}</div>}
    </div>
  );
}
